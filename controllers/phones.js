...
getAll = async (req, res) => {
    const result = await mongodb.getDb().db().collection("phones").find();
...
getSingle = async (req, res) => {
    if (!ObjectId.isValid(req.params.id)) {
        res.status(400).json('Must use a valid contact id to find a phone.');
        return
      }
...
createPhone = async (req, res) => {
    ...
    if (response.acknowledged) {
        ...
        res.status(500).json(
            response || "Some error occured while trying to create phone."
        );
    }
};
...
updatePhone = async (req, res) => {
    ...
    if (response.modifiedCount > 0) {
        ...
        res.status(500).json(
            response.error || "Some error occured while trying to update phone."
        );
    }
};
...
deletePhone = async (req, res) => {
    ...
    if (response.deletedCount > 0) {
        ...
        res.status(500).json(
            response || "Some error occured while trying to delete phone."
        );
    }
};
