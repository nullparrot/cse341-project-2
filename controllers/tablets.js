...
getAll = async (req, res) => {
    const result = await mongodb.getDb().db().collection("tablets").find();
...
getSingle = async (req, res) => {
    if (!ObjectId.isValid(req.params.id)) {
        res.status(400).json('Must use a valid contact id to find a tablet.');
        return
      }
...
createTablet = async (req, res) => {
    ...
    if (response.acknowledged) {
        ...
        res.status(500).json(
            response || "Some error occured while trying to create tablet."
        );
    }
};
...
updateTablet = async (req, res) => {
    ...
    const response = await mongodb
        ...
        .db()
        .collection("tablets")
        .replaceOne({ _id: tabletId }, tablet);
    if (response.modifiedCount > 0) {
        res.status(204).send();
    } else {
        res.status(500).json(
            response.error ||
                "Some error occured while trying to update tablet."
        );
    }
};
...
deleteTablet = async (req, res) => {
    ...
    const response = await mongodb
        ...
        .db()
        .collection("tablets")
        .deleteOne({ _id: tabletId });
    if (response.deletedCount > 0) {
        res.status(200).send();
    } else {
        res.status(500).json(
            response || "Some error occured while trying to delete tablet."
        );
    }
};
